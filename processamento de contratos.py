...
def validar_conteudo(df):
    ...
    for i, row in df.iterrows():
        for col in df.columns:
            ...
            if any(c in valor for c in ['Ç', 'Á', 'É', 'Í', 'Ó', 'Ú', 'À', 'Â', 'Ê', 'Ô', 'Õ', 'Ã', 'Ü', 'Ñ', "'", "’"]):
                erros.append(f"Linha {i+2} - {col}: contém caractere proibido.")

            if col in valores_validos and valor not in valores_validos[col]:
                erros.append(f"Linha {i+2} - {col}: valor inválido ({valor})")

            if col == 'CEP' and not re.match(regex_validos['CEP'], valor):
                erros.append(f"Linha {i+2} - CEP: formato inválido ({valor})")

            if col == 'CPF' and not re.match(regex_validos['CPF'], valor):
                erros.append(f"Linha {i+2} - CPF: formato inválido ({valor})")

            if col in ['DATA_NASCIMENTO', 'DATA BASE'] and not re.match(regex_validos['DATA'], valor):
                erros.append(f"Linha {i+2} - {col}: data inválida ({valor})")

            if col in ['FONE_RESIDENCIA', 'FONE_COMERCIAL'] and valor and not re.match(regex_validos['TELEFONE'], valor):
